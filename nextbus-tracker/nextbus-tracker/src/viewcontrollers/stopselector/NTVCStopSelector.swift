...
class NTVCStopSelector: UIViewController, MKMapViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func btnToggleListAct(_ sender: UIBarButtonItem) {
        if (sender.title == "Map") {
            sender.title = "List"
    ...
    func prepareMapView() {
        ...
        for path in routePaths {
            let coordinates = UnsafeMutablePointer<CLLocationCoordinate2D>.allocate(capacity: path.count)
            ...
            coordinates.deallocate(capacity: path.count)
        }
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if (annotation is NTMAStops) {
            ...
            var annotationView = mapPaths.dequeueReusableAnnotationView(withIdentifier: reuseId)
            ...
            btn.addTarget(self, action: #selector(self.annotationBtnAct(_:)), for: UIControlEvents.touchUpInside)
    ...
    @objc func annotationBtnAct(_ sender: UIButton) {
        let indexPath = IndexPath(row: sender.tag, section: 0)
        self.tableView(tblStops, didSelectRowAt: indexPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell: NTTblCellStops = tblStops.dequeueReusableCell(withIdentifier: "tblCellStops") as! NTTblCellStops
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let stopTag = stops[indexPath.row][NTMNextbus.NTMKeyTag] as? String {
            if let stopTitle = stops[indexPath.row][NTMNextbus.NTMKeyTitle] as? String {
                let b = NTMNextbus.addStopToLocalStorage(agency: NTMNextbus.NTMDefaultAgency, route: routeTag, routeTitle: routeTitle, direction: "loop", directionTitle: "Loop", stop: stopTag, stopTitle: stopTitle)
